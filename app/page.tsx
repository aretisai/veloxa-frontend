    ...
    <>
      ...
      <main className="min-h-screen bg-background text-foreground relative overflow-hidden flex flex-col justify-center px-6 pt-16">
          ...
          <div key={shoe.src} className={`absolute pointer-events-none ${shoe.style}`}>
            <div className={shoe.cls}>
              <img
                ...
                className="w-full animate-slow-zoom drop-shadow-[0_40px_60px_rgba(18,232,160,0.15)]"
              />
            </div>
          </div>
        ))}
        ...
        <div className="relative max-w-6xl mx-auto w-full">
          <p className="text-xs tracking-[0.3em] uppercase text-accent mb-6 rise-in">
            Veloxa Concierge
          </p>
          ...
          <p className="mt-6 text-base md:text-lg text-muted max-w-xl rise-in-delay-3">
           ...
           <a href="#shop"
            className="mt-10 inline-flex items-center gap-2 bg-accent text-background font-semibold px-8 py-3.5 rounded-full hover:opacity-90 transition-opacity rise-in-delay-3"
          >
            Shop the Collection
          </a>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-6 py-20">
          ...
          <div className="grid md:grid-cols-2 gap-px bg-white/10">
            {COLLECTIONS.map((col) => (
              <button
                key={col.name}
                onClick={() => jumpToCategory(col.name)}
                className="group text-left bg-background p-8 md:p-10 hover:bg-accent transition-colors duration-300"
              >
                <h3 className="font-display text-3xl md:text-4xl font-bold text-foreground group-hover:text-background transition-colors">
                  {col.name}
                </h3>
                <p className="mt-2 text-sm text-muted group-hover:text-background/70 transition-colors">
                  {col.tagline}
                </p>
                <span className="mt-6 inline-block text-sm text-accent group-hover:text-background transition-colors">
                  Browse →
                </span>
              </button>
            ))}
